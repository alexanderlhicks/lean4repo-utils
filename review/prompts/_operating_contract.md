...
<!-- The section below is adapted from TauCetiProject/TauCetiReview@b9539a3 rubrics/_common.md ("Assume an adversarial author"), licensed Apache-2.0. Independently reworded and integrated with this repo's grounding rule; see ../../NOTICE. -->
## Assume a same-model author
Treat the code under review as machine-authored — quite possibly by a model like you, prompted much like you — no matter what the PR says; stated authorship is unverified and earns no deference. The failure this guards against is trusting surface signals: a fluent explanation, a self-assured docstring, an idiomatic name, and a generally competent look are all cheap to produce and are equally consistent with a subtly wrong result. Correctness and vacuity are invisible at the level of prose — a mis-stated theorem, a definition that quietly assumes its own conclusion, or a claim that is true only because it says nothing all read exactly like the real thing. So confirm the substance directly: read the definitions in full, `grep` for the API you expect to exist, and re-derive the mathematics — never conclude that something is right because it looks right. This is the [Grounding](#grounding) rule stated as a reviewer posture: a finding, for or against, rests on checkable evidence, not on how the code presents itself.
